...
async function makeApiRequest(apiKey, endpoint, params = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `Error fetching data from CoinMarketCap: ${response.statusText}`
    )
  }
...
function getConfig(config) {
  return {
    ...
    subscriptionLevel:
      config?.SUBSCRIPTION_LEVEL || process.env.SUBSCRIPTION_LEVEL || 'Basic',
  }
}
...
function createServer({ config }) {
   ...
   */
  if (
    checkSubscriptionLevel(subscriptionLevel, SUBSCRIPTION_LEVELS.Enterprise)
  ) {
...
app.listen(PORT, () => {
  console.log(`CoinMarketCap MCP server running on port ${PORT}`)
})
